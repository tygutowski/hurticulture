...
@export var basic_heightmap : FastNoiseLite
@export var complex_heightmap : FastNoiseLite
		...
		for xOffset in range(-chunksvisible,chunksvisible):
			...
			if terrain_chunks.has(view_chunk_coord):
				...
				if terrain_chunks[view_chunk_coord].update_lod(viewer_position):
					terrain_chunks[view_chunk_coord].generate_terrain(mountain_noise,
																	  view_chunk_coord,
																	  chunkSize,
																	  true)
...
#					last_visible_chunks.append(terrain_chunks[view_chunk_coord])
				...
				chunk.generate_terrain(mountain_noise, 
									   view_chunk_coord,
									   chunkSize,
									   false)
